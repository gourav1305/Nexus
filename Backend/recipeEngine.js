...
function parseTimeTrigger(config) {
  ...
  if (type === 'interval') return { kind: 'interval', ms: parseInt(config.intervalMs) || 60000 };
  if (type === 'daily') {
...
function isTimeToRun(config, lastRun) {
  ...
  const now = Date.now();
  if (cfg.kind === 'interval') return !lastRun || (now - lastRun) >= cfg.ms;
  if (cfg.kind === 'daily') return !lastRun || lastRun < cfg.target;
  if (cfg.kind === 'hourly') return !lastRun || lastRun < cfg.target;
  return false;
}
...
async function executeAction(action, recipe, { runPowerShell, logEvent, apiUsage, recipeMessages }) {
  ...
  const interpolate = (str) =>
    str.replace(/\{\{recipeName\}\}/g, recipe.name)
  ...
  switch (action.type) {
    case 'message': {
      ...
      if (recipeMessages) recipeMessages.push({ role: 'system', text, timestamp: Date.now() });
      if (logEvent) logEvent('recipe', `Recipe "${recipe.name}" sent message`, text);
      return { result: 'message_sent', text };
    }
    case 'command': {
      const cmd = interpolate(action.config.command || '');
      if (!cmd) return { result: 'error', error: 'No command specified' };
      try {
        const output = await runPowerShell(cmd);
        if (logEvent) logEvent('recipe', `Recipe "${recipe.name}" ran command`, cmd);
        if (apiUsage) apiUsage.systemCommands++;
        return { result: 'command_executed', output: output.stdout?.slice(0, 500) };
      } catch (err) {
        if (logEvent) logEvent('recipe', `Recipe "${recipe.name}" command failed`, err.message);
        return { result: 'error', error: err.message };
      }
    }
    case 'api': {
      const url = interpolate(action.config.url || '');
      if (!url) return { result: 'error', error: 'No URL specified' };
      try {
        ...
        if (logEvent) logEvent('recipe', `Recipe "${recipe.name}" called API`, `${method} ${url}`);
        return { result: 'api_called', status: response.status, data: data.slice(0, 500) };
      } catch (err) {
        if (logEvent) logEvent('recipe', `Recipe "${recipe.name}" API call failed`, err.message);
        return { result: 'error', error: err.message };
      }
    }
    case 'blob': {
      ...
      if (logEvent) logEvent('recipe', `Recipe "${recipe.name}" changed blob`, JSON.stringify(payload));
...
function stopFileWatcher(recipeId) {
  if (activeWatchers.has(recipeId)) { activeWatchers.get(recipeId).close(); activeWatchers.delete(recipeId); }
}
...
function startFileWatcher(recipe, onTrigger) {
  ...
  try {
    ...
    const watcher = fs.watch(watchTarget, (eventType, filename) => {
      ...
      if (targetFile && filename && !filename.includes(path.basename(targetFile))) return;
      const event = recipe.trigger.config.event || 'change';
      if (event !== 'all' && eventType !== event) return;
...
function createRecipeEngine({ app, runPowerShell, logEvent, apiUsage, recipeMessages }) {
  ...
  function logRecipeEvent(recipeId, message, detail) {
    ...
    if (recipeLog.length > MAX_RECIPE_LOG) recipeLog.splice(0, recipeLog.length - MAX_RECIPE_LOG);
  }
  ...
  function checkTimeTriggers() {
    ...
    for (const recipe of recipes) {
      if (recipe.trigger.type !== 'time') continue;
  ...
  function pollEmailTriggers() {
    const recipes = getAllEnabledRecipes();
    for (const recipe of recipes) {
      if (recipe.trigger.type !== 'email') continue;
      if (!recipe.trigger.config.host || !recipe.trigger.config.username) continue;
      triggerRecipe(recipe.id, recipe.userId);
    }
  }

  function registerWebhookRoute(recipe) {
    if (registeredWebhooks.has(recipe.id)) return;
    const hookPath = `/api/webhook/${recipe.id}`;
    app.post(hookPath, async (req, res) => {
      const fresh = db.getRecipeById(recipe.id, recipe.userId);
      if (!fresh || !fresh.enabled) return res.status(404).json({ error: 'Recipe not found or disabled' });
      const secret = fresh.trigger.config.secret;
      if (secret && req.headers['x-webhook-secret'] !== secret) return res.status(401).json({ error: 'Invalid secret' });
  ...
  function getRecipeLog(recipeId) {
    if (recipeId) return recipeLog.filter(e => e.recipeId === recipeId).slice(-50);
  ...
  function start() {
    const recipes = getAllEnabledRecipes();
    for (const recipe of recipes) {
      if (recipe.trigger.type === 'file') startFileWatcher(recipe, triggerRecipe);
      if (recipe.trigger.type === 'webhook') registerWebhookRoute(recipe);
    }
...
module.exports = { createRecipeEngine };
