...
exports.toggleRecipeById = toggleRecipeById;
exports.updateRecipeRun = updateRecipeRun;
...
function getChatHistory(userId, limit = 100) {
    return getDb().prepare('SELECT id, role, text, timestamp FROM chat_history WHERE user_id = ? ORDER BY timestamp ASC, id ASC LIMIT ?').all(userId, limit);
}
function addChatEntry(userId, role, text) {
    const stmt = getDb().prepare('INSERT INTO chat_history (user_id, role, text, timestamp) VALUES (?, ?, ?, ?)');
...
function getRecipes(userId) {
    const rows = getDb().prepare('SELECT * FROM recipes WHERE user_id = ? ORDER BY created_at DESC').all(userId);
...
function createRecipe(userId, data) {
    ...
    VALUES (?, ?, ?, ?, 1, ?, ?, ?, ?, ?)
  `).run(id, userId, data.name || 'Unnamed', data.description || '', data.trigger?.type || 'time', JSON.stringify(data.trigger?.config || {}), data.action?.type || 'message', JSON.stringify(data.action?.config || {}), Date.now());
...
function updateRecipeById(id, userId, data) {
  ...
  `).run(data.name ?? existing.name, data.description ?? existing.description, data.trigger?.type ?? existing.trigger_type, JSON.stringify(data.trigger?.config ?? safeJson(existing.trigger_config)), data.action?.type ?? existing.action_type, JSON.stringify(data.action?.config ?? safeJson(existing.action_config)), id, userId);
    return getRecipeById(id, userId);
}
...
function getTodos(userId) {
    return getDb().prepare('SELECT * FROM todos WHERE user_id = ? ORDER BY created_at DESC').all(userId).map(normalizeTodo);
}
...
function updateTodo(id, userId, updates) {
    ...
    WHERE id = ? AND user_id = ?
  `).run(updates.text ?? existing.text, updates.completed !== undefined ? (updates.completed ? 1 : 0) : existing.completed, updates.priority ?? existing.priority, updates.dueDate ?? existing.due_date, id, userId);
