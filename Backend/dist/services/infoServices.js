...
exports.getInfoApiStatus = exports.handleInfoQuery = exports.detectInfoQuery = void 0;
...
const extractCity = (text) => {
    ...
    const match = text.match(/\b(?:in|at|for|of|ka|ki|ke|me|mein|par)\s+([A-Za-z][A-Za-z\s]{1,24}?)(?:\s+(?:ka|ki|ke|me|mein|par|weather|mausam|temperature|temp|rain|aaj|today|abhi|now)\b|$)/i);
    if (match?.[1]) {
...
const extractNewsCategory = (text) => {
    const normalized = text.toLowerCase();
    if (/\b(tech|technology|it|ai|startup)\b/.test(normalized))
        return 'technology';
    if (/\b(sport|sports|cricket|football|ipl)\b/.test(normalized))
        return 'sports';
    if (/\b(business|finance|market|stock|economy)\b/.test(normalized))
        return 'business';
    if (/\b(health|medical|science)\b/.test(normalized))
        return 'health';
    if (/\b(entertainment|bollywood|movie|film)\b/.test(normalized))
...
const extractTopic = (text) => {
    ...
    if (keBaareMatch?.[1])
        return keBaareMatch[1].trim();
    const bareMeMatch = text.match(/(.+?)\s+bare\s+me\b/i);
    if (bareMeMatch?.[1])
        return bareMeMatch[1].trim();
    ...
    if (bataoMatch?.[1] && bataoMatch[1].length > 2)
        return bataoMatch[1].trim();
    return text
        .replace(/\b(please|mujhe|mujhko|tell me|batao|bata|bataye|explain|information|info)\b/gi, ' ')
        .replace(/\s+/g, ' ')
        .trim();
};
...
const getWeatherWttr = async (cityQuery) => {
    ...
    return (`${place} ka weather: ${description}, temperature ${temp}°C, feels like ${feels}°C. `
...
const getWeatherOpenWeather = async (cityQuery) => {
    ...
    return (`${cityName} ka weather: ${description}, temperature ${temp}°C, feels like ${feels}°C. `
...
const getBraveSearch = async (query) => {
    ...
    if (!results.length)
        throw new Error('No Brave results');
...
const getSerperSearch = async (query) => {
    ...
    if (!results.length)
        throw new Error('No Serper results');
    const lines = results.slice(0, 5).map((item, index) => {
        const title = item.title || 'Result';
...
const detectInfoQuery = (message) => {
    const text = message
        ...
        .replace(/\s+/g, ' ')
        .trim();
    if (/\b(weather|mausam|temperature|temp|rain|barish|garmi|sardi|humidity|forecast)\b/.test(text)
        || /\bweather\s+kya\s+hai\b/.test(text)
        || /\bmausam\s+kya\s+hai\b/.test(text)) {
        return { type: 'weather', city: extractCity(message) };
    }
    if (/\b(search|google|find|lookup|dhundo|khojo|web\s+search)\b/.test(text)
...
exports.detectInfoQuery = detectInfoQuery;
...
exports.handleInfoQuery = handleInfoQuery;
...
exports.getInfoApiStatus = getInfoApiStatus;
