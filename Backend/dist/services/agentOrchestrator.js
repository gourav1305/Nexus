...
exports.detectTaskRequest = detectTaskRequest;
const systemCommander = require("./systemCommander");
...
function detectTaskRequest(text) {
    ...
    if (/\b(?:agent|autonomous|autonomously|task|apne aap|khud se|automatically)\s+(?:se|karo|kare|do|execute|run|handle|banao|create|make|build)\b/i.test(text))
        return true;
    if (/^(?:agent|task|autonomous|apne aap|khud)\b/i.test(text.trim()))
        return true;
    return false;
}
