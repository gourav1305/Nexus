...
function guessLanguage(code) {
    if (/^\s*(import |from |def |class |print|#!|if __name__)/m.test(code) || /\b(input\(|print\()/m.test(code))
        return 'python';
    if (/^\s*(const |let |var |function|import |export |console\.|require\()/m.test(code) || /=>\s*{/.test(code))
        return 'javascript';
    if (/^\s*(#!\/bin\/bash|echo |export |alias |if \[|for i in |while true|case\s+\$)/m.test(code))
        return 'bash';
    return 'python';
}
...
function runDirect(language, code, onOutput, onError, onDone) {
    ...
    let outputBuffer = '';
    let settled = false;
    const settle = (fn, arg) => { if (!settled) {
        settled = true;
        fn(arg);
    } };
    const cmds = {
...
async function runDocker(language, code, onOutput, onError, onDone) {
    ...
    let outputBuffer = '';
    const flushOutput = () => { if (outputBuffer) {
        onOutput(outputBuffer);
        outputBuffer = '';
    } };
    child.stdout.on('data', (chunk) => {
