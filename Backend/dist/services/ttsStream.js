...
exports.createTtsStreamHandler = createTtsStreamHandler;
    ...
    wss.on('connection', (ws) => {
        ws.on('message', async (raw) => {
            ...
            if (msg.type === 'speak') {
                ...
                if (!text)
                    return ws.send(JSON.stringify({ error: 'No text' }));
                const voicePrefs = (0, voiceCatalog_1.resolveVoicePrefs)(msg.voicePrefs || {});
...
`;
        ...
        child.on('close', (code) => {
            if (code !== 0)
                return reject(new Error(stderr || 'edge-tts stream failed'));
...
`;
        ...
        child.on('close', (code) => {
            if (code !== 0 || !stdout)
                return reject(new Error(stderr || 'SAPI stream failed'));
