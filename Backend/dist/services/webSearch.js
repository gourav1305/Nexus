...
async function searchWeb(query, numResults = 5) {
    ...
    if (SERPAPI_KEY) {
        try {
            const serpRes = await axios_1.default.get('https://serpapi.com/search', {
                params: {
                    q: query,
                    api_key: SERPAPI_KEY,
                    num: numResults,
            ...
            for (const item of organic.slice(0, numResults)) {
                ...
                if (item.snippet)
                    results.snippets.push(item.snippet);
            }
            return results;
        }
        catch (err) {
    ...
    try {
        const ddgRes = await axios_1.default.get('https://api.duckduckgo.com/', {
    ...
    try {
        const htmlRes = await axios_1.default.get('https://www.google.com/search', {
