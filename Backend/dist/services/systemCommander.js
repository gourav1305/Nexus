...
exports.clipboardGetHistory = clipboardGetHistory;
...
const launchProcess = (command, args = [], options = {}) => new Promise((resolve, reject) => {
    ...
    let stdout = '';
    let stderr = '';
    const settle = (callback, value) => { if (!settled) {
        settled = true;
        callback(value);
    } };
    const timer = setTimeout(() => settle(reject, new Error(`Timed out after ${TIMEOUT}ms`)), options.timeout || TIMEOUT);
    if (captureOutput) {
        child.stdout?.on('data', (chunk) => { stdout += chunk.toString(); if (stdout.length > MAX_OUTPUT)
            child.kill(); });
        child.stderr?.on('data', (chunk) => { stderr += chunk.toString(); if (stderr.length > MAX_OUTPUT)
            child.kill(); });
    }
    child.once('error', (error) => { clearTimeout(timer); settle(reject, error); });
    child.once('exit', (code) => { clearTimeout(timer); if (!captureOutput)
        return settle(resolve, { stdout, stderr, code }); settle(resolve, { stdout, stderr, code }); });
    child.once('spawn', () => { if (!captureOutput) {
        clearTimeout(timer);
        child.unref();
        settle(resolve, { stdout: '', stderr: '', code: 0 });
    } });
});
    ...
    try {
        ...
        if (action === 'open' || action === 'navigate') {
            ...
            const snippets = await page.evaluate(() => {
                ...
                const items = Array.from(document.querySelectorAll('.g .VwiC3b, .g .lEBKkf, [data-sncf], .rc .s, .BNeawe.s3v9rd.AP7Wnd, .BNeawe.UPmit'));
                return items.slice(0, 5).map(el => el.textContent).filter(Boolean);
            });
...
async function analyzeImage(imagePath, query) {
    if (!modelRouter)
        throw new Error('Vision model not available (modelRouter not initialized)');
    if (!fs.existsSync(imagePath))
        throw new Error('Image file not found: ' + imagePath);
    const ext = path.extname(imagePath).toLowerCase();
    if (!['.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp'].includes(ext))
        throw new Error('Not a supported image format: ' + ext);
    ...
    if (text) {
        ...
        if (!last || last.text !== text) {
            ...
            if (clipboardHistory.length > 50)
                clipboardHistory.splice(0, clipboardHistory.length - 50);
        }
    }
...
async function extractDocumentText(filePath) {
    ...
    if (ext === '.pdf') {
        try {
            // @ts-ignore - pdf-parse module type quirk
...
async function executeTool(toolCall) {
    ...
    if (!type || !action)
        throw new Error('Invalid tool call: missing type or action');
    switch (type) {
        case 'file':
            switch (action) {
                ...
                case 'write':
                case 'create':
                case 'make': return await writeFile(params.path, params.content);
        ...
        case 'system':
            ...
            if (action === 'clipboard-write')
                return await clipboardWrite(params);
            if (action === 'clipboard-history')
                return await clipboardGetHistory();
            if (action === 'read-document')
                return await extractDocumentText(params.path);
            return await systemControl(action, params);
        case 'vision':
            if (action === 'analyze')
                return await analyzeImage(params.path, params.query);
