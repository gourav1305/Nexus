...
let serverVoiceSettings = (0, voiceCatalog_1.resolveVoicePrefs)({
    voiceMode: voiceCatalog_1.DEFAULT_VOICE_MODE,
    speakingRate: voiceCatalog_1.DEFAULT_SPEAKING_RATE,
    voice: TTS_VOICE,
});
...
app.get('/api/web/search', async (req, res) => {
    try {
        ...
        if (!q || !q.trim())
            return res.status(400).json({ ok: false, error: 'Query param "q" required' });
        ...
        const results = await (0, webSearch_1.searchWeb)(q, num);
...
const detectSystemCommand = (message) => {
    ...
    if (text.includes('file explorer')
...
const ttsEngine_1 = require("./ttsEngine");
const infoServices_1 = require("./services/infoServices");
const recipeEngine_1 = require("./recipeEngine");
const auth_1 = require("./auth");
const ttsStream_1 = require("./services/ttsStream");
const emotionDetector_1 = require("./services/emotionDetector");
const webSearch_1 = require("./services/webSearch");
const memoryStore_1 = __importDefault(require("./services/memoryStore"));
const codeRunner_1 = require("./services/codeRunner");
const emailService = __importStar(require("./services/emailService"));
const imageGen = __importStar(require("./services/imageGen"));
const translationService = __importStar(require("./services/translationService"));
const voiceLab = __importStar(require("./services/voiceLab"));
app.get('/api/voices', (req, res) => {
    res.json({
        ok: true,
        modes: voiceCatalog_1.VOICE_MODES,
        rates: voiceCatalog_1.SPEAKING_RATES,
        default: {
            voiceMode: voiceCatalog_1.DEFAULT_VOICE_MODE,
            speakingRate: voiceCatalog_1.DEFAULT_SPEAKING_RATE,
...
app.post('/api/settings/voice', (req, res) => {
    try {
        serverVoiceSettings = (0, voiceCatalog_1.resolveVoicePrefs)(req.body || {}, TTS_VOICE);
        res.json({
            ok: true,
            settings: serverVoiceSettings,
        });
...
app.post('/api/settings/models', (req, res) => {
    try {
        ...
        if (provider)
            serverModelPrefs.provider = provider;
        if (model)
            serverModelPrefs.model = model;
        if (visionModel)
            serverModelPrefs.visionModel = visionModel;
        if (autoRoute !== undefined)
            serverModelPrefs.autoRoute = Boolean(autoRoute);
        res.json({ ok: true, settings: serverModelPrefs });
    }
    catch (error) {
...
app.get('/api/info/status', (req, res) => {
    res.json({
        ok: true,
        apis: (0, infoServices_1.getInfoApiStatus)(),
    });
});
app.get('/api/tools', (req, res) => {
    res.json({
        ...
        infoApis: (0, infoServices_1.getInfoApiStatus)(),
...
const recipeEngine = (0, recipeEngine_1.createRecipeEngine)({
    app,
    runPowerShell,
    logEvent,
    apiUsage,
...
app.post('/api/chat/vision', auth_1.optionalAuth, async (req, res) => {
    try {
        const { message, imageBase64, imageMimeType, voice, rate, voiceMode, speakingRate, language } = req.body || {};
        const voicePrefs = (0, voiceCatalog_1.resolveVoicePrefs)({ voice, rate, voiceMode, speakingRate, language }, serverVoiceSettings.voice);
        if (!imageBase64) {
        ...
        const speech = await (0, ttsEngine_1.synthesizeSpeech)(textResponse, voicePrefs, TTS_VOICE);
        apiUsage.ttsCalls++;
        res.json({
            text: textResponse,
            ...speech,
...
app.post('/api/chat', auth_1.optionalAuth, async (req, res) => {
    try {
        ...
        if (pendingSystemAction && Date.now() - pendingSystemAction.time < COMMAND_TIMEOUT) {
            ...
            if (isConfirmed) {
                ...
                try {
                    ...
                    const speech = await (0, ttsEngine_1.synthesizeSpeech)(confirmReply, voicePrefs, TTS_VOICE);
                    apiUsage.ttsCalls++;
                    ...
                    const speech = await (0, ttsEngine_1.synthesizeSpeech)(failReply, voicePrefs, TTS_VOICE);
                    apiUsage.ttsCalls++;
                ...
                const speech = await (0, ttsEngine_1.synthesizeSpeech)(cancelReply, voicePrefs, TTS_VOICE);
                apiUsage.ttsCalls++;
                return res.json({ text: cancelReply, ...speech });
            }
        }
        const infoQuery = (0, infoServices_1.detectInfoQuery)(message);
        if (infoQuery) {
            try {
                const infoReply = await (0, infoServices_1.handleInfoQuery)(infoQuery);
                const speech = await (0, ttsEngine_1.synthesizeSpeech)(infoReply, voicePrefs, TTS_VOICE);
                ...
                if (req.userId) {
                    try {
                        await memoryStore_1.default.add(groq, req.userId, 'user', 'user', message.trim());
                        await memoryStore_1.default.add(groq, req.userId, 'nexus', 'assistant', infoReply);
                    }
                    catch { }
                }
                return res.json({
        ...
        if (systemCommand) {
            ...
            const speech = await (0, ttsEngine_1.synthesizeSpeech)(systemCommand.reply, voicePrefs, TTS_VOICE);
            apiUsage.ttsCalls++;
            if (!systemCommand.requiresConfirmation) {
                apiUsage.systemCommands++;
            }
        ...
        if (agentOrchestrator.detectTaskRequest(message)) {
            try {
                ...
                const taskResult = await agentOrchestrator.processTask(message.trim(), serverModelPrefs, (type, msg) => {
                    if (type === 'log')
                        console.log('[Agent]', msg);
                });
                const taskSummary = taskResult.summary;
                const speech = await (0, ttsEngine_1.synthesizeSpeech)(taskSummary, voicePrefs, TTS_VOICE);
                apiUsage.ttsCalls++;
                apiUsage.llmCalls++;
                return res.json({
                    text: taskSummary,
        ...
        const emotion = (0, emotionDetector_1.detectEmotion)(message.trim());
        let systemContent = (0, emotionDetector_1.buildSystemPrompt)('You are NEXUS, a concise voice assistant. Reply naturally in the same language or Hinglish style as the user. Keep responses voice-friendly and avoid markdown unless absolutely needed.', emotion);
        if (emotion)
            logEvent('emotion', `Detected: ${emotion.emotion} (score: ${emotion.score})`);
        ...
        if (needsMemory || needsWebSearch) {
            ...
            if (needsMemory && userId) {
                try {
                    const memories = await memoryStore_1.default.query(groq, userId, message, 5);
            ...
            if (needsWebSearch) {
                ...
                if (searchQuery.length > 3) {
                    try {
                        const webResults = await (0, webSearch_1.searchWeb)(searchQuery, 3);
...
app.post('/api/execute', express_1.default.raw({ type: '*/*', limit: '64kb' }), async (req, res) => {
    res.writeHead(200, {
    ...
    try {
        ...
        await (0, codeRunner_1.executeCode)({
            language,
            code,
...
app.post('/api/tts/preview', async (req, res) => {
    try {
        ...
        if (!voice)
            return res.status(400).json({ ok: false, error: 'Voice name is required' });
...
app.post('/api/system/:action', async (req, res) => {
    try {
        ...
        switch (action) {
            case 'file-read':
                result = await systemCommander.readFile(params.path);
                break;
            case 'file-write':
                result = await systemCommander.writeFile(params.path, params.content);
                break;
            case 'file-list':
                result = await systemCommander.listDir(params.path);
                break;
            case 'project-create':
                result = await systemCommander.createProject(params.name, params.structure);
                break;
            case 'git-status':
                result = await systemCommander.gitRun('status', params.path);
                break;
            case 'git-log':
                result = await systemCommander.gitRun('log', params.path);
                break;
            case 'git-commit':
                result = await systemCommander.gitRun('commit', params.path, { message: params.message });
                break;
            case 'git-diff':
                result = await systemCommander.gitRun('diff', params.path);
                break;
            case 'git-push':
                result = await systemCommander.gitRun('push', params.path);
                break;
            case 'git-pull':
                result = await systemCommander.gitRun('pull', params.path);
                break;
            case 'git-branch':
                result = await systemCommander.gitRun('branch', params.path);
                break;
            case 'browser':
                result = await systemCommander.browserAction(params.action || 'open', params);
                break;
            case 'system-info':
                result = await systemCommander.systemControl('system-info');
                break;
            case 'system-volume':
                result = await systemCommander.systemControl('volume', params);
                break;
            case 'system-brightness':
                result = await systemCommander.systemControl('brightness', params);
                break;
            case 'system-open':
                result = await systemCommander.systemControl('open-app', params);
                break;
            case 'system-processes':
                result = await systemCommander.systemControl('process-list');
                break;
            case 'system-kill':
                result = await systemCommander.systemControl('kill-process', params);
                break;
            case 'file-search':
                result = await systemCommander.executeTool({ type: 'system', action: 'search-files', params });
                break;
            case 'clipboard-read':
                result = await systemCommander.executeTool({ type: 'system', action: 'clipboard-read' });
                break;
            case 'clipboard-write':
                result = await systemCommander.executeTool({ type: 'system', action: 'clipboard-write', params });
                break;
            case 'clipboard-history':
                result = await systemCommander.executeTool({ type: 'system', action: 'clipboard-history' });
                break;
            case 'read-document':
                result = await systemCommander.executeTool({ type: 'system', action: 'read-document', params });
                break;
...
app.post('/api/task', async (req, res) => {
    try {
        ...
        const result = await agentOrchestrator.processTask(message.trim(), serverModelPrefs, (type, msg) => {
            if (type === 'log') {
                const isStep = msg.match(/Step (\d+)\/(\d+)/);
                if (isStep)
                    sendEvent('step', { current: parseInt(isStep[1]), total: parseInt(isStep[2]), text: msg });
                else
                    sendEvent('log', { text: msg });
            }
        });
        sendEvent('done', result);
        res.end();
...
const wss = new ws_1.WebSocketServer({ server, path: '/ws/tts' });
(0, ttsStream_1.createTtsStreamHandler)(wss, logEvent);
...
server.listen(PORT, () => {
    ...
    console.log('Info APIs:', (0, infoServices_1.getInfoApiStatus)());
});
const shutdown = () => {
    recipeEngine.stop();
