...
function optionalAuth(req, res, next) {
    ...
    if (header && header.startsWith('Bearer ')) {
        try {
            const decoded = jsonwebtoken_1.default.verify(header.split(' ')[1], JWT_SECRET);
            req.userId = decoded.id;
            req.username = decoded.username;
        }
        catch { }
...
router.post('/register', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.id, username: user.username }, JWT_SECRET, { expiresIn: JWT_EXPIRES });
        res.status(201).json({
            ok: true,
            token,
