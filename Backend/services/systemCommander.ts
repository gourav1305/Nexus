...
async function gitRun(action: any, repoPath: any, options: any = {}) {
...
async function browserAction(action: string, params: any = {}) {
  if (!puppeteer) {
  ...
  try {
    ...
    if (action === 'open' || action === 'navigate') {
      ...
      const snippets = await page.evaluate(() => {
        ...
        const items: any[] = Array.from(document.querySelectorAll('.g .VwiC3b, .g .lEBKkf, [data-sncf], .rc .s, .BNeawe.s3v9rd.AP7Wnd, .BNeawe.UPmit'));
...
async function systemControl(action: string, params: any = {}) {
  if (action === 'volume') {
